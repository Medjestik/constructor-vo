...
function AssessmentAbilityList({ ability, onOpen }) {
      ...
      <div className='table__header'>
        <div className='table__main-column table__main-column_type_empty'>
          ...
          <div className='table__column table__column_type_header table__column_type_medium'>
            <p className='table__text table__text_type_header table__text_align_center'>Задания</p>
          </div>
        </div>
      </div>
              ...
              <div className='table__main-column'>
                ...
                <div className='table__column table__column_type_large'>
                  <ul className='table__status-list'>
                    {basisCount.map((elem) => (
                      <li className={`table__status-item ${elem <= item.tasks_count ? 'table__status-item_type_active' : ''}`} key={elem + '-' + item.id}></li>
                    ))}
                  </ul>
                </div>
                <div className='table__column table__column_type_medium'>
                  <p className='table__text table__text_align_center'>{item.tasks_count || 0}</p>
                </div>
              </div>
            </li>
          ))
...
export default AssessmentAbilityList;  