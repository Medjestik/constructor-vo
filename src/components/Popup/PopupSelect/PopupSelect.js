  ...
  return (
    <div className={`select-popup ${isOpenSelectOptions ? 'select-popup_status_open' : ''}`}>
      <div className='select-popup__main' onClick={openSelectOptions}>
        <p className={`select-popup__text ${currentOption.id === 'placeholder' ? 'select-popup__text_type_placeholder' : ''}`}>{currentOption.name}</p>
      ...
      <div className={`select-popup__options-container ${isOpenSelectOptions ? 'select-popup__options-container_status_open' : ''}`}>
        <ul className='select-popup__options-list scroll'>
          {
            options.filter(item => item.id !== currentOption.id && item.id !== 'placeholder').map((item, i) => (
